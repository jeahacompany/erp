...
(function () {
  ...
  function fetchPage(page) {
    var lastUrl = '';
    return window.__bjNet.get(url(page))
      .then(function (r) { lastUrl = r.url || ''; return r.text(); })
      .then(function (html) {
        // ⚠ 로그인 풀림은 세 가지 모습으로 온다. 크기로 판단하면 안 된다.
        //   ① /Login/ 으로 끌려감 ② 로그인 화면 HTML(29KB) ③ 98바이트 <script> 조각

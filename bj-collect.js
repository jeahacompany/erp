...
(function () {
  ...
  var DAYTYPES = (opt.daytypes && opt.daytypes.length)
               ...
               //
               // ⚠ 2026-09-07 — **순서가 곧 "언제 메워지나" 다.**
               //   송장일을 앞에 두었더니, 한 축을 통째로 끝내야 다음 축이 시작되므로
               //   37일치 송장일을 다 돌 때까지 주문일은 시작조차 못 했다.
               //   그런데 그날 누락 292건은 전부 **주문일 축**이었다.
               //   주문일이 "아직 송장 안 붙은 주문" 까지 보는 넓은 축이다. 이쪽을 먼저 돈다.
               : (opt.sync ? ['order_date', 'deliveryNum_updatedate'] : [DAYTYPE]);
  var DRY = !!opt.dryRun;
  ...
  if (opt.sync) {
    ...
    ask({ type: 'BJ_PLAN_REQ', kind: KIND, source: 'orders', daytypes: DAYTYPES },
      ...
      .then(function (r) {
        ...
        //   ⚠ 처음긁어오기는 **알람 주기(10분)보다 짧게** 잡는다.
        //     12분으로 뒀더니 10분마다 시작하는 처음긁어오기가 끊임없이 이어져
        //     최근분을 담당하는 빠른수집이 **한 번도 끼어들지 못했다** (2026-09-07).
        //     그래서 9/5·9/6 매출이 몇 시간째 안 들어왔다. 틈을 남겨 둔다.
        // ⚠ 2026-09-07 밤 — 과거자료 채우기를 위해 6분 → 12분.
        //   왜 12분인가: 확장의 자물쇠 제한이 15분이다(SYNC_MAX_MS['처음 긁어오기']).
        //   예산이 그 제한을 넘으면 확장이 먼저 작업을 끊어 버리고,
        //   페이지 쪽 수집기만 주인 없이 계속 도는 상태가 된다. **제한보다 짧아야 한다.**
        //   그리고 최근자료가 급하면 8분 뒤 자리를 빼앗아 가므로(SYNC_URGENT)
        //   과거 채우기가 오늘 자료를 막지 않는다.
        var BUDGET_MS = KIND === 'backfill' ? 12 * 60 * 1000
                      : KIND === 'deep' ? 12 * 60 * 1000
                      : 8 * 60 * 1000;
